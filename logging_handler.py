        ...
        for key, value in args.items():
            if key == "content" and isinstance(value, str):
                if len(value) > MAX_TOOL_ARG_LENGTH:
                    ...
                    print(
                        f"    📄 {key}: (panjang {len(value)} karakter, cuplikan: '{preview}')"
                    )
                else:
...
def log_tool_output(function_name, output_result):
    ...
    if (
        isinstance(output_result, dict)
        and "success" in output_result
        and "data" in output_result
    ):
        ...
        print(
            f"{Colors.YELLOW}└─ Hasil {function_name} (Format tidak standar): {Colors.RESET}"
        )
