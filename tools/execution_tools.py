...
def execute_command(command, args=None, timeout=60):
    ...
    try:
        ...
        return json.dumps(
            {
                "success": False,
                "data": f"Error: Perintah '{shlex_join(full_command)}' melebihi batas waktu {timeout} detik.",
            }
        )
        ...
        return json.dumps(
            {
                "success": False,
                "data": f"Gagal mengeksekusi perintah '{shlex_join(full_command)}': {e}",
            }
        )
    ...
    try:
        ...
        if pip3_result["success"]:
            return json.dumps(
                {
                    "success": True,
                    "data": f"Paket '{package_name}' berhasil diinstal menggunakan pip3. Output: {pip3_result['data']}",
                }
            )
        ...
        if pip_result["success"]:
            return json.dumps(
                {
                    "success": True,
                    "data": f"Paket '{package_name}' berhasil diinstal menggunakan pip. Output: {pip_result['data']}",
                }
            )
        ...
        return json.dumps(
            {
                "success": False,
                "data": f"Terjadi kesalahan saat mencoba menginstal paket '{package_name}': {e}",
            }
        )
    ...
    try:
        ...
        if not isinstance(mode, str) or not mode.isdigit() or not (1 <= len(mode) <= 4):
            return json.dumps(
                {
                    "success": False,
                    "data": "Error: Mode harus berupa string digit oktal (contoh: '755', '644').",
                }
            )

        os.chmod(full_path, int(mode, 8))
        return json.dumps(
            {"success": True, "data": f"Izin untuk '{path}' berhasil diatur ke {mode}."}
        )
    except ValueError as e:
        return json.dumps({"success": False, "data": f"Kesalahan keamanan: {e}"})
    except FileNotFoundError:
        return json.dumps(
            {"success": False, "data": f"Jalur '{path}' tidak ditemukan."}
        )
    except Exception as e:
        return json.dumps(
            {"success": False, "data": f"Gagal mengatur izin untuk '{path}': {e}"}
        )
...
execution_tool_definitions = glm.Tool(
    function_declarations=[
        glm.FunctionDeclaration(
            ...
            parameters=glm.Schema(
                ...
                properties={
                    ...
                    "timeout": glm.Schema(type=Type.NUMBER),
                },
                required=["command"],
            ),
        ),
        glm.FunctionDeclaration(
            ...
            parameters=glm.Schema(
                type=Type.OBJECT,
                properties={"package_name": glm.Schema(type=Type.STRING)},
        ...
        glm.FunctionDeclaration(
            ...
            parameters=glm.Schema(
                type=Type.OBJECT,
                properties={
                    "path": glm.Schema(type=Type.STRING),
                    "mode": glm.Schema(type=Type.STRING),
                },
                required=["path", "mode"],
            ),
        ),
